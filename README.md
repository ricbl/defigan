...
This code reproduces part of the results presented in the paper [Interpretation of Disease Evidence for Medical Images Using Adversarial Deformation Fields](https://arxiv.org/abs/...), to be presented at [MICCAI 2020](https://www.miccai2020.org/en/). This code implementation is done in Linux, using Python 3.7.3 and PyTorch 1.2, and was partially inspired by [Orobix VAGAN code](https://github.com/orobix/Visual-Feature-Attribution-Using-Wasserstein-GANs-Pytorch). Part of the code is modified from [Baumgartner's VAGAN code](https://github.com/baumgach/vagan-code).

The DeFI-GAN algorithm can be used to generate spatial explanations of disease evidence. An adversarially trained generator produces deformation fields that modify images of diseased patients to resemble images of healthy patients. The deformation field presents can present, by the use of quiver plots, changes in structure position, shape, and size. These changes represent what the method found as evidence of disease in the dataset and can also be used to evaluate unexpected dataset biases. The DeFI-GAN method presented higher normalized cross-correlation (NCC) scores than a baseline ([VA-GAN](https://arxiv.org/abs/1711.08998)). NCC was calculated between disease effect maps generated by the methods and disease effect maps calculated using longitudinal data. The following image is an overview of the DeFI-GAN method. For more details, consult the [paper](https://arxiv.org/abs/...).

![](https://github.com/ricbl/defigan/raw/master/readme_images/defigan.png)

From the datasets used in the paper, the COPD dataset is private and is not available for outside investigators. The ADNI dataset has [limited access](http://adni.loni.usc.edu/data-samples/access-data/) to use the data is approved. Downloading and preprocessing the ADNI dataset as used in the paper may take several weeks. We also provide a toy dataset for quick testing of the algorithm without worrying about setting up data. The first run of the code will automatically generate the synthetic dataset. 

## Setup

To install all the needed libraries, you can use the `requirements.sh` file. It assumes you have [conda or miniconda](https://docs.conda.io/en/latest/) installed and creates a conda environment called `defigan`, with prerequisites installed. Activate the environment before running the code using `conda activate defigan`. In case you want to run the method for the ADNI dataset, after running the bash file install the SimpleITK library following [these instructions](https://simpleelastix.readthedocs.io/GettingStarted.html). After compiling it, make sure you have the conda environment `defigan` activated when following the instructions in the Python module installation section. The compilation may take a couple hours, but SimpleITK is not necessary for playing with the toy dataset. For details on how to set up the ADNI dataset, go to the "ADNI data setup" section at the end of this README.

## Usage

This section provides what command line commands you should use to train and test the proposed method, DeFI-GAN, and the baseline used for comparison, VA-GAN. To reproduce the image provided in results, run the "Test pre-trained model". All commands are using the GPU indexed by 0, but you can change the argument `gpus`  according to your needs. For the test commands, replace the `<timestamps-id>` expression with the respective value of the training experiment. You can run `python -m src.train --help` to see all available options for modifying the training and test scripts. All commands should be run from the project base folder. To check test NCC scores, open the log.txt file inside the experiment folder (`./runs/...`). The first time a dataset is used, H5 files are created for faster loading of datasets in subsequent runs, so the first run may take an unusually long time to start producing outputs.

### Toy dataset

This dataset contains centered squares of different sizes. Larger squares are considered "healthy" (class 0) and smaller squares are considered "abnormal" (class 1). Squares are generated in a fixed size and have a random smoothed Gaussian noise added to its inside. Then squares are resized according to a random side length following a Weibull distribution. An arbitrary threshold on the side length is used to define the class of the square. Ground truths for the differences between an "abnormal" square and its equivalent "healthy" example are calculated by resizing the square and then performing a subtraction. For this dataset, a batch size of 8 needs a GPU with at least 8 GB of memory.
...
To run the baseline for this dataset, you will need a GPU with at least 11 GB of memory. The difference in memory usage between methods for this dataset comes mainly from the framework (PyTorch vs. Tensorflow) and the training inference batch size (4 vs. 2). For the Tensorflow VA-GAN implementation, the effective batch size in terms of gradient step is 12, since it uses gradient accumulation of 6 training inferences for each gradient step. Other peculiarities of this implementation include: 
* using $\hat{x}_{0} = \tanh(x_1+G(x_1))$, in place of $\hat{x}_{0} = x_1+G(x_1)$;
* a learning rate of $1\mathrm{e}{-3}$.

## Results
For the toy dataset, DeFI-GAN gets an NCC of ~0.7 in the validation set, while VA-GAN gets an NCC of ~0.5. We did not perform hyperparameter optimization, and these results are just for illustration of the code. For the other datasets, expected results are in the table below:
...
<table>
<tr>
<td>Dataset</td> <td> NCC </td><td width="170px"> Input image<br><img src="https://render.githubusercontent.com/render/math?math=(x)"> </td><td width="170px"> Desired change<br><img src="https://render.githubusercontent.com/render/math?math=(\Delta x)">  </td><td width="170px">  Deformation Field<br><img src="https://render.githubusercontent.com/render/math?math=(\phi)"> </td><td width="170px"> Produced change<br><img src="https://render.githubusercontent.com/render/math?math=(\widehat{\Delta x})"> </td><td width="170px"> <img src="https://render.githubusercontent.com/render/math?math=\widehat{\Delta x}"> over <img src="https://render.githubusercontent.com/render/math?math=x"></td><td width="170px"> Modified image<br><img src="https://render.githubusercontent.com/render/math?math=(\hat{x}_0)"></td>
</tr>

<td>Toy </td><td> 0.710 </td><td> <img src="https://github.com/ricbl/defigan/raw/master/readme_images/flow/toy/x1.png" alt="drawing" min-width="170px" max-width="170px" width="170px"/>   </td><td>  <img src="https://github.com/ricbl/defigan/raw/master/readme_images/flow/toy/difference_gt.png" alt="drawing" min-width="170px" max-width="170px" width="170px"/>   </td><td> <img src="https://github.com/ricbl/defigan/raw/master/readme_images/flow/toy/flow_quiver.png" alt="drawing" min-width="170px" max-width="170px" width="170px"/>   </td><td> <img src="https://github.com/ricbl/defigan/raw/master/readme_images/flow/toy/difference.png" alt="drawing" min-width="170px" max-width="170px" width="170px"/>  </td><td> <img src="https://github.com/ricbl/defigan/raw/master/readme_images/flow/toy/difference_overlaid.png" alt="drawing" min-width="170px" max-width="170px" width="170px"/>  </td><td> <img src="https://github.com/ricbl/defigan/raw/master/readme_images/flow/toy/xhat0.png" alt="drawing" min-width="170px" max-width="170px" width="170px"/> </td></tr>
<tr>
<td>
ADNI (coronal view) </td><td> 0.369 </td><td> <img src="https://github.com/ricbl/defigan/raw/master/readme_images/flow/adni/x1.png" alt="drawing" min-width="170px" max-width="170px" width="170px"/>   </td><td>  <img src="https://github.com/ricbl/defigan/raw/master/readme_images/flow/adni/difference_gt.png" alt="drawing" min-width="170px" max-width="170px" width="170px"/>   </td><td> <img src="https://github.com/ricbl/defigan/raw/master/readme_images/flow/adni/flow_quiver.png" alt="drawing" min-width="170px" max-width="170px" width="170px"/>   </td><td> <img src="https://github.com/ricbl/defigan/raw/master/readme_images/flow/adni/difference.png" alt="drawing" min-width="170px" max-width="170px" width="170px"/>  </td><td> <img src="https://github.com/ricbl/defigan/raw/master/readme_images/flow/adni/difference_overlaid.png" alt="drawing" min-width="170px" max-width="170px" width="170px"/>  </td><td> <img src="https://github.com/ricbl/defigan/raw/master/readme_images/flow/adni/xhat0.png" alt="drawing" min-width="170px" max-width="170px" width="170px"/> </td>
</tr>
</table>
#### Baseline (VA-GAN - addition)

Dataset | NCC | Input image<br><img src="https://render.githubusercontent.com/render/math?math=(x)"> | Desired change<br><img src="https://render.githubusercontent.com/render/math?math=(\Delta x)"> | Produced change<br><img src="https://render.githubusercontent.com/render/math?math=(\widehat{\Delta x})"> | <img src="https://render.githubusercontent.com/render/math?math=\widehat{\Delta x}"> over <img src="https://render.githubusercontent.com/render/math?math=x">| Modified image<br><img src="https://render.githubusercontent.com/render/math?math=(\hat{x}_0)">
--- | --- | --- | --- | --- |  --- | ---
...
All the outputs of the model are saved in the `runs` folder, inside a folder for the specific experiment you are running (`<experiment name>_<timestamp>`). These are the files that are saved:
  ...
  * xhat0_\<epoch\>: modified input image (sum of original image and delta_x) at the end of that epoch for the fixed set of examples
  * flow_quiver_\<epoch\>: quiver plot of the deformation vector field given by the generator. If the option `scale_flow_arrow` is not modified, the length of the arrows is given in pixels. Only saved if `generator_out`=flow.
  ...
  * delta_x_\<epoch\>.png: delta_x output of the generator at the end of that epoch for the fixed set of examples. Only non-zero if `generator_out`=residual.
  * difference_\<epoch\>: difference between xhat0 and x1
  * difference_overlaid_\<epoch\>: the difference between xhat0 and x1 colored with a pink/green colormap, overlaid over the x1.
  ...
  * critic_state_dict_\<epoch\>: checkpoint for the critic model
...
* Downloas the brain MRI atlas, [mni_icbm152_t1_tal_nlin_asym_09a.nii](https://neurovault.org/images/29400/).
...
* Run the script by using `python -m src.adni_preprocess_all`. This preprocessing may take a long time (in the order of weeks if running on only one computer). It might be advantageous to divide the data into different folders and run the script in several servers in parallel. Results will then be saved to `<bmicdatasets_root>/Processed_part_2/ADNI_all_no_PP_3`.
* Edit the value of variable `Processed_part_2_folder` in the  `correct_brain_processing.py` script to point to the location of the `Processed_part_2`folder. Run the `correct_brain_processing.py` script. Results will be saved to `<bmicdatasets_root>/Processed_part_3/ADNI_all_no_PP_3`.
* Check every resulting file for outliers in terms of file size. Sometimes preprocessing will fail, and, in most cases, it will produce either a file larger than usual (when the skull was not completely removed) or smaller than usual (when more parts of the brain were removed than desired). Rerun all these steps for all corrupted files you found.
* For some volumes, even rerunning the whole preprocessing will give wrong results. These volumes can be ignored by the algorithm by including them in `images_where_preprocessing_failed`, a list defined in `brain_loader.py`, following the format `<rid>_<viscode>`. Seven volumes were removed in this step for the results of the DeFI-GAN paper (855_0, 424_4, 4741_2, 4741_1, 4741_5, 4741_7, 4892_0).
