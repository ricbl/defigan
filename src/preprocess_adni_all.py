...
INCLUDE_MISSING_IMAGES_IN_TABLE = False
DO_ONLY_TABLE = False

bmicdatasets_root = '~/ADNI/'
bmicdatasets_originals = os.path.join(bmicdatasets_root, '')
bmicdatasets_adni = os.path.join(bmicdatasets_originals, '')
bmicdatasets_adni_tables = os.path.join(bmicdatasets_adni, 'Tables/')

bmidatasets_mni = os.path.join(bmicdatasets_originals, 'atlas/')
...
N4_executable = '~/bin/ants_centos7/bin/N4BiasFieldCorrection'
robex_executable = '~/ROBEX/runROBEX.sh'
...
def do_preprocessing(adnimerge_table_arg,
                     ...
                     write_csv=True, bmicdatasets_adni_images=None):
        ...
        for ii, row in adnimerge_table_arg.iterrows():
            ...
            for exam_date, field_strength in zip(exam_dates, field_strengths):
                ...
                for preproc_subfolder in preproc_subfolders:
                    ...
                    print(os.path.join(bmicdatasets_adni_images, nii_search_str))
                ...
                if image_exists:
                    ...
                    if not DO_ONLY_TABLE:
                        ...
                        if not do_postprocessing:
                            ...
                            tmp_file_path2 = os.path.join(tmp_file_folder, 'tmp_rid%s_%s2.nii.gz' % (str(rid).zfill(4), str(tmp_index)))
                            ...
                            # fsl orientation enforcing:
                                ...
                                Popen('fslreorient2std {0} {1}'.format(tmp_file_path, tmp_file_path2), shell=True).communicate()
                                try:
                                    os.remove(tmp_file_path)
                                except FileNotFoundError:
                                    pass
                                try:
                                    shutil.copyfile(tmp_file_path2, tmp_file_path)
                                except FileNotFoundError:
                                    logging.info('ID6542130: %s' % (nii_use_file))
                                    continue
                                try:
                            ...
                            if do_cropping:
                                ...
                                Popen('robustfov -i {0} -r {1}'.format(tmp_file_path, tmp_file_path2), shell=True).communicate()
                                try:
                                    os.remove(tmp_file_path)
                                except FileNotFoundError:
                                    pass
                                try:
                                    ...
                                    logging.info('ID6542131: %s' % (nii_use_file))
                                    continue
                                try:
                                    os.remove(tmp_file_path2)
                                except FileNotFoundError:
                            ...
                            if do_bias_correction:
                                ...
                                Popen('{0} -i {1} -o {2}'.format(N4_executable, tmp_file_path, tmp_file_path2),
                                ...
                                try:
                                    ...
                                    logging.info('ID6542132: %s' % (nii_use_file))
                                    continue
                                try:
                                    os.remove(tmp_file_path2)
                                except FileNotFoundError:
                            ...
                            if do_registration:
                                ...
                                Popen(
                                    'flirt -in {0} -ref {1} -out {2} -searchrx -45 45 -searchry -45 45 -searchrz -45 45 -dof 7'.format(
                                        tmp_file_path, mni_template_t1, tmp_file_path2), shell=True).communicate()
                                try:
                                    os.remove(tmp_file_path)
                                except FileNotFoundError:
                                    pass
                                try:
                                    ...
                                    logging.info('ID6542133: %s' % (nii_use_file))
                                    continue
                                try:
                                    os.remove(tmp_file_path2)
                                except FileNotFoundError:
                            ...
                            if do_skull_stripping:
                                ...
                                Popen('{0} {1} {2} -R -f 0.5 -g 0'.format(robex_executable, tmp_file_path, tmp_file_path2), shell=True).communicate()
                                try:
                                    os.remove(tmp_file_path)
                                except FileNotFoundError:
                                    pass
                                try:
                                    ...
                                    logging.info('ID6542134: %s' % (nii_use_file))
                                    continue
                                try:
                                    os.remove(tmp_file_path2)
                                except FileNotFoundError:
...
def main(bmicdatasets_adni_images, processed_images_folder):
    ...
    do_preprocessing(adnimerge_table, 0,
                     ...
                     write_csv=True, bmicdatasets_adni_images = bmicdatasets_adni_images)

    logging.info('Elapsed time %f secs' % (time.time()-start_time))
    
if __name__ == '__main__':
    main(os.path.join(bmicdatasets_adni, 'ADNI/'), os.path.join(bmicdatasets_root, 'Processed_part_2/ADNI_all_no_PP_3'))
