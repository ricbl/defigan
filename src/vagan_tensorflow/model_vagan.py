...
import os
from . import test_pretrained_vagan_tensorflow
...
class vagan:
    ...
    def __init__(self, exp_config, data, fixed_batch_size=None):
        ...
        self.opt = test_pretrained_vagan_tensorflow.get_opt_from_command('--gpus='+os.environ['CUDA_VISIBLE_DEVICES']+' --batch_size='+str(self.exp_config.batch_size)+' --folder_dataset='+self.exp_config.preproc_folder+' --dataset_to_use=adni --experiment='+self.exp_config.experiment_name+' --split_validation=val --n_dimensions_data=3')
        self.data = data
    ...
    def critic_loss(self):
        ...
        """
        self.l_rx = tf.reduce_mean(self.D) 
        self.l_rxprime = - tf.reduce_mean(self.D_)
        
        cri_loss = self.l_rx + self.l_rxprime
        ...
        if self.exp_config.improved_training:
            ...
            self.l_r_gp = tf.reduce_mean(tf.square(slopes - 1.0))
            slopes_corrected = tf.sqrt(tf.reduce_sum(tf.square(grads), axis=[1,2,3]))
            self.l_r_gp_corrected = tf.reduce_mean(tf.square(slopes_corrected - 1.0))
            penalty = self.exp_config.scale * self.l_r_gp_corrected
    ...
    def _make_tensorboard_summaries(self):
        ...
        tf.summary.scalar('l_rx_', self.l_rx)
        tf.summary.scalar('l_r_gp_', self.l_r_gp)
        tf.summary.scalar('l_r_gp_corrected', self.l_r_gp_corrected)
        tf.summary.scalar('l_rxprime_', self.l_rxprime)
        tf.summary.scalar('l_gxprime_', self.l_gxprime)
        tf.summary.scalar('l_reg_', self.l1_map_reg)
        tf.summary.scalar('regressor_loss_', self.cri_loss)
        tf.summary.scalar('critic_loss', self.cri_loss)
        tf.summary.scalar('gen_loss_', self.gen_loss)
        tf.summary.scalar('generator_loss', self.gen_loss)
        tf.summary.scalar('total_loss_', self.gen_loss + self.cri_loss)
    ...
    def _do_validation_and_save_model(self, step):
        ...
        ncc = test_pretrained_vagan_tensorflow.main(self.opt,'', self, step)
        c1_imgs = self.data.validationAD.next_batch(self.exp_config.batch_size)[0]
        c0_imgs = self.data.validationCN.next_batch(self.exp_config.batch_size)[0]
        ...
        summary = tf.Summary()
        summary.value.add(tag = 'ncc', simple_value = ncc)
        self.summary_writer.add_summary(summary, step)
        self.summary_writer.flush()

        logging.info("[Validation], generator loss: %g, critic_loss: %g" % (total_g_loss_val, total_d_loss_val))

        if ncc >= self.best_ncc:
            self.best_ncc = ncc

            logging.info("Found new best NCC! Saving model...")
